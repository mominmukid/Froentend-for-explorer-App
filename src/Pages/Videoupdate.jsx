...
function VideoUpdatePage() {
    ...
    const oldVideoDetails = async () => {
      try {
        ...
        if (fetchAsyncVideoSingle.fulfilled.match(resultAction)) {
          ...
          setThumbnail(videoData.thumbnailUrl); // Assuming the API returns a URL for the thumbnail
        } else {
  ...
  const showConfirm = (message, onAccept) => {
    confirmDialog({
      ...
      accept: onAccept,
      rejectClassName:
        "px-4 py-2 rounded-lg bg-gray-300 text-gray-900 hover:bg-gray-400 transition",
      acceptClassName:
        "px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition",
      className:
        "rounded-2xl shadow-lg border border-gray-200 bg-white p-6 w-[90%] sm:w-[400px]",
    ...
    showConfirm("Are you sure you want to update the details?", async () => {
      try {
        ...
        const resultAction = await dispatch(
          updateVideoDetails({ id, title, description })
        );
        ...
        console.error(error);
        toast.error("Details update failed. Please try again.", {
          autoClose: 2000,
          theme: "dark",
        });
      } finally {
  ...
  return (
    <div className="min-h-screen ctext-gray-900 dark:text-gray-200 p-4 sm:p-8">
